...
class SessionRecorder:
    ...
    def __init__(self):
        ...
        # Directed follow relations between agents (edges)
        self.following_relations: List[dict] = []
    ...
    def record_agent_action(self, agent_id: str, position: List[int],
                            ...
                            rank: int = 999, iteration: int = 0,
                            follow_action: dict = None) -> dict:
        ...
        # Attach follow_action and update following_relations graph
        if follow_action:
            agent_data["follow_action"] = follow_action
            edge = {
                "from_agent": agent_id,
                "to_agent": follow_action.get("target_agent_id"),
                "from_position": position,
                "to_position": follow_action.get("target_position"),
                "since_iteration": follow_action.get("since_iteration", iteration),
                "relation_type": follow_action.get("relation_type", ""),
                "is_new": follow_action.get("is_new", False),
                "is_end": follow_action.get("is_end", False),
                "timestamp": agent_data["timestamp"],
            }
            self.following_relations.append(edge)
            # Limit history to avoid unbounded growth
            if len(self.following_relations) > 2000:
                self.following_relations = self.following_relations[-2000:]
    ...
    try:
        ...
        if msg_type == 'quantum_snapshot':
            ...
            export_data = {
                ...
                'following_relations': session_recorder.following_relations
            }
