...
class AIPlayerV6 {
  constructor() {
    ...
    // Follow / entanglement state between W-agents
    // followings: agents that THIS agent currently follows (by id)
    // followers:  agents that follow THIS agent (filled when server sends it later)
    this.followings = {};          // { agentId: { position: [x,y], sinceIteration, relationType } }
    this.followers = {};           // { agentId: { position: [x,y], sinceIteration, relationType } }
    this.currentFollowing = null;  // current followed agentId (for persistence)
    this.followingSinceIteration = null; // local iteration when currentFollowing started
    // Local default persistence (can later be driven by external config)
    this.followPersistenceMinIter = 3;
    this.lastFollowAction = null;  // last follow_action descriptor sent to servers
  ...
  log(...args) {
    const msg = args.join(' ');
    // Filtrer le bruit récurrent
    const skipPatterns = [
      'Snapshot pending, waiting',
      'Global image generated',
      'Burst sent',
      'W-data sent',
      'Image sent to O+N server'
    ];
    ...
    if (this.elements.journal) {
      // Supprimer le placeholder initial s’il existe
      const firstChild = this.elements.journal.firstElementChild;
      if (firstChild && firstChild.classList.contains('journal-entry') && firstChild.textContent.includes('Waiting for connection')) {
        firstChild.remove();
      }
      ...
      this.elements.journal.appendChild(entry);

      // Limiter la taille du journal pour éviter l’inflation
      const maxEntries = 300;
      while (this.elements.journal.children.length > maxEntries) {
        this.elements.journal.removeChild(this.elements.journal.firstChild);
      }
  ...
  async executeSeed(ctx) {
    ...
    // Log compact pour l’Event Log (Debug)
    {
      const posX = this.myPosition?.[0] ?? '-';
      const posY = this.myPosition?.[1] ?? '-';
      this.log(`Agent [${posX},${posY}] | It. ${this.iterationCount} | strategy: Quantum seed generation`);
    }
  ...
  async executeAction(ctx) {
    ...
    const richCtx = {
      ...
      // Follow state (will be updated after parsing, for next iteration prompts)
      current_following_agent_id: this.currentFollowing,
      current_following_position: this.currentFollowing ? (this.followings[this.currentFollowing]?.position || null) : null,
      following_since_iteration: this.followingSinceIteration,
    ...
    // Update follow state based on parsed response (if relational strategy)
    this.updateFollowStateFromParsed(parsed);
    ...
    // Log compact pour l’Event Log (Debug)
    {
      const posX = this.myPosition?.[0] ?? '-';
      const posY = this.myPosition?.[1] ?? '-';
      const strategyLabel = parsed.strategy || strategyId || 'N/A';
      this.log(`Agent [${posX},${posY}] | It. ${this.iterationCount} | strategy: ${strategyLabel}`);
    }
  ...
  // === Follow / entanglement helpers ===

  getAgentIdByPosition(pos) {
    if (!pos || pos.length !== 2) return null;
    const [x, y] = pos;
    // Check other users first
    for (const [uid, data] of Object.entries(this.otherUsers)) {
      const p = data.position;
      if (Array.isArray(p) && p[0] === x && p[1] === y) {
        return uid;
      }
    }
    // Also allow following self (rare but possible)
    if (this.myPosition[0] === x && this.myPosition[1] === y) {
      return this.myUserId;
    }
    return null;
  }

  updateFollowStateFromParsed(parsed) {
    try {
      const previousFollowing = this.currentFollowing;
      let action = null;

      // Expect optional source_agents: array of positions [[x,y], ...]
      const sourceAgents = parsed?.source_agents;

      // Case 1: no relational source_agents → end of current following if any
      if (!Array.isArray(sourceAgents) || sourceAgents.length === 0) {
        if (previousFollowing) {
          const info = this.followings[previousFollowing] || {};
          const pos = info.position || null;
          const relationType = info.relationType || 'relation';
          action = {
            target_agent_id: previousFollowing,
            target_position: pos,
            relation_type: relationType,
            is_new: false,
            is_end: true,
            since_iteration: info.sinceIteration ?? this.followingSinceIteration ?? this.iterationCount
          };
          this.currentFollowing = null;
          this.followingSinceIteration = null;
        }
        this.lastFollowAction = action;
        return;
      }

      const primary = sourceAgents[0];
      if (!Array.isArray(primary) || primary.length !== 2) {
        return;
      }

      const targetPos = [parseInt(primary[0], 10), parseInt(primary[1], 10)];
      const targetId = this.getAgentIdByPosition(targetPos);
      if (!targetId) {
        return;
      }

      const relationType = parsed?.strategy_id || (parsed?.strategy_ids && parsed.strategy_ids[0]) || 'relation';

      // If we already follow this agent, just ensure metadata is up to date
      if (this.currentFollowing === targetId) {
        const existing = this.followings[targetId] || {};
        const sinceIter = existing.sinceIteration ?? this.followingSinceIteration ?? this.iterationCount;
        this.followings[targetId] = {
          position: targetPos,
          sinceIteration: sinceIter,
        ...
        action = {
          ...
          is_new: false,
          is_end: false,
          since_iteration: sinceIter
        };
        this.lastFollowAction = action;
        return;
      }

      // Start following a new agent (no persistence enforcement here – handled by prompt rules)
      this.currentFollowing = targetId;
      this.followingSinceIteration = this.iterationCount;
      this.followings[targetId] = {
        position: targetPos,
        sinceIteration: this.iterationCount,
        relationType: relationType
      };
      ...
      action = {
        ...
        is_new: true,
        is_end: false,
        since_iteration: this.iterationCount
      };

      this.lastFollowAction = action;
    } catch (e) {
      console.warn('[V6-Q] updateFollowStateFromParsed error:', e);
    }
  }
  ...
  async sendWData(data) {
    ...
    try {
      ...
      // Attach follow_action descriptor if available
      if (this.lastFollowAction) {
        payload.follow_action = this.lastFollowAction;
      }
  ...
  calculateSignallingTokens(outputTokens, parsed) {
    ...
    // Estimation des tokens "mécaniques" SANS pixels :
    ...
    // - Quelques valeurs numériques de deltas (~6 tokens)
    ...
    const mechanicalWithoutPixels =
      baseStructureTokens +
      fieldNamesTokens +
      deltaStructureTokens +
      deltaValueTokens;
    
    // Tokens pour l'encodage COMPLET des pixels (x,y#HEX)
    // Approximé à ~5 tokens par pixel (coordonnées + virgules + '#'+HEX)
    const pixelCount = Array.isArray(parsed?.pixels) ? parsed.pixels.length : 0;
    const PIXEL_TOKENS_PER_PIXEL = 5;
    const pixelTokens = pixelCount * PIXEL_TOKENS_PER_PIXEL;
    
    // Tokens de signalement pur = tout ce qui n'est ni structurel ni pixels
    const signallingPure = Math.max(
      0,
      outputTokens - mechanicalWithoutPixels - pixelTokens
    );
    
    return {
      total: outputTokens,
      mechanical: mechanicalWithoutPixels,
      pixel: pixelTokens,
      signalling: signallingPure
    };
  }
  ...
  async sendWDataToMetrics(parsed, iteration, tokens = null, signallingTokens = null) {
    ...
    try {
      ...
      // Attach follow_action descriptor if available
      if (this.lastFollowAction) {
        metricsData.follow_action = this.lastFollowAction;
      }
