// Position Canvas Generator - Génère des images du canvas global
// Utilise le même système de coordonnées que viewer2 (centré avec offset)
// Génère 2 types d'images :
// 1. Canvas pur (artistique, couleur)
// 2. Structure (noir & blanc, grilles + bordure épaisse)

export class PositionCanvasGenerator {
    /**
     * Génère les images du canvas global
    ...
    static generatePureCanvas(otherUsers, myUserId) {
        ...
        console.log('[Position Canvas] otherUsers:', Object.keys(otherUsers).length, 'agents');
        ...
        console.log(`[Position Canvas] Positions: [${minX},${minY}] to [${maxX},${maxY}], taille: ${gridSideSize}×${gridSideSize}`);
        ...
        // IMPORTANT: Utiliser la même logique que viewer2
        // viewer2 utilise: offset = Math.floor(gridSize / 2)
        // puis: pixelX = (x + offset) * cellSize
        const offset = Math.floor(gridSideSize / 2);
        ...
        for (const [userId, userData] of Object.entries(otherUsers)) {
            ...
            // Position relative à l'offset (comme viewer2)
            const baseX = (pos[0] + offset) * cellSize;
            const baseY = (pos[1] + offset) * cellSize;
        ...
        console.log(`[Position Canvas PURE] Généré canvas ${canvas.width}×${canvas.height} avec ${Object.keys(otherUsers).length} grilles`);
    ...
    static generateStructureCanvas(otherUsers, myUserId) {
        ...
        for (const [userId, userData] of Object.entries(otherUsers)) {
            ...
            if (userId === myUserId) {
                ...
                console.log(`[Position Canvas STRUCTURE] Bordure épaisse pour user ${userId} à (${pos[0]}, ${pos[1]})`);
            }
        }
        ...
        console.log(`[Position Canvas STRUCTURE] Généré canvas N&B ${canvas.width}×${canvas.height}`);
