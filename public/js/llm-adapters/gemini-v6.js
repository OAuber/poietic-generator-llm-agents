...
export const GeminiV6Adapter = {
  ...
  async buildSystemPrompt(kind, context) {
    ...
    // FOLLOWING STATE (for action)
    let currentFollowingText = 'None';
    let followingsHistoryText = '';
    const persistenceMinIter = 3;

    if (kind === 'action') {
      const cfId = context?.current_following_agent_id || '';
      const cfPos = context?.current_following_position || null;
      const cfSince = context?.following_since_iteration;

      if (cfId) {
        const posStr = cfPos && Array.isArray(cfPos) ? `[${cfPos[0]},${cfPos[1]}]` : '[?,?]';
        currentFollowingText = `Agent ${cfId.substring(0, 8)} at ${posStr} since iteration ${cfSince ?? '?'}`;
      }

      // Compress followings map into a short history string if available
      if (context?.followings && typeof context.followings === 'object') {
        const entries = Object.entries(context.followings)
          .slice(-5)
          .map(([aid, info]) => {
            const pos = info.position || ['?','?'];
            const rel = info.relationType || 'relation';
            const sinceIt = info.sinceIteration ?? '?';
            return `${aid.substring(0, 8)}@[${pos[0]},${pos[1]}](${rel},it${sinceIt})`;
          });
        followingsHistoryText = entries.join('; ');
      }
    }
    ...
    const render = (s) => s
      ...
      .replaceAll('{{current_following}}', currentFollowingText)
      ...
      .replaceAll('{{followings_history}}', followingsHistoryText)
      ...
      // Follow persistence
      .replaceAll('{{following_persistence_min_iter}}', String(persistenceMinIter));
